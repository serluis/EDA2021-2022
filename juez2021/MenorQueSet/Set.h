...
public:
    ...
    //devuelve si this.set esta contenido en set2
    bool operator<=(const Set<T>& set2) const {//coste O(n) siendo n el tamaño nelems mas corto entre this.nelems y set2.nelems
        if (this->empty()) {
            return true;
        }
        else if (set2.empty()) {
            ...
            int i = 0, e = 0;
            while (i < this->nelems && e < set2.nelems) {
                
                if (this->array[i] > set2.array[e]) {
                    e++;
                }
                else if (this->array[i] == set2.array[e]) {
                    i++; e++;
                }
                else {
                    return false;
            ...
            if (e >= set2.nelems && i < this->nelems) {
                return false;
            }
            return true;
        }
